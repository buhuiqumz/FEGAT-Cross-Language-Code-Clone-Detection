...
# from graphnnSiamese3 import graphnn3
import json
import pandas as pd
import os
...
def get_f_name_AST():
    ...
    F_NAME.append(r'java-cpp-9-9.json')
    return F_NAME


def get_f_dict_AST(F_NAME):
    name_num = 0
    name_dict = {}
...
def read_graph_AST(F_NAME, FUNC_NAME_DICT, FEATURE_DIM,n_num):
    graphs = []
    classes = []
...
def partition_data_AST(Gs, classes, partitions, perm):
    C = len(classes)
    st = 0.0
    ret = []
...
def generate_epoch_pair_AST(Gs, classes, M, output_id=False, load_id=None):
    epoch_data = []
...
def train_epoch(model, graphs, classes, batch_size, load_data=None):
    if load_data is None:
        epoch_data = generate_epoch_pair_AST(graphs, classes, batch_size)
    else:
...
def get_auc_epoch_AST(model, graphs, classes, batch_size, load_data=None):
    tot_diff = []
    tot_truth = []
