...
def graph_embed(X, msg_mask, N_x, N_embed, N_o, iter_level, Wnode, Wembed, W_output, b_output, Wattention):
    # 计算节点的初始值
    ...
    for t in range(iter_level):
        ...
        attention_input = tf.tensordot(cur_msg, Wattention[t], axes=[[2], [0]])  # [batch, node_num, embed_dim]
        attention_output = tf.matmul(msg_mask, attention_input)  # [batch, node_num, embed_dim]

        tot_val_t = node_val + attention_output
...
class graphnn3(object):
    def __init__(self,
        ...
        with tf.device(device):
            ...
            Wattention = []
            for i in range(depth_embed):
                Wattention.append(tf.Variable(tf.truncated_normal(
                    shape=[N_embed, N_embed], stddev=0.1, dtype=Dtype)))
            ...
            # Wattention = tf.Variable(tf.truncated_normal(
            #     shape=[N_embed, N_embed], stddev=0.1, dtype=Dtype))


            X1 = tf.placeholder(Dtype, [None, None, N_x])  # [B, N_node, N_x]
            msg1_mask = tf.placeholder(Dtype, [None, None, None])  # [B, N_node, N_node]
            self.X1 = X1
            self.msg1_mask = msg1_mask


            embed1 = graph_embed(X1, msg1_mask, N_x, N_embed, N_o, ITER_LEVEL,
                                 Wnode, Wembed, W_output, b_output, Wattention)
